...
API access for scripts, services, and integrations is managed through **API keys**—client ID and secret pairings created in the console. A key authenticates your _application_ rather than a person, so it works without a signed-in user and never expires with a session.
