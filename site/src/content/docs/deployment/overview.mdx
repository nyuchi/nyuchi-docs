...
- Docker container images on Vercel Functions — see
  [Agent Gateway on Vercel](/deployment/agentgateway/) for the pattern we
  use to ship `nyuchi/agentgateway` as a `Dockerfile.vercel` image.
